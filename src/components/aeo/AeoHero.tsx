...
interface AeoHeroProps {
  darkMode: boolean;
}

export default function AeoHero({ darkMode }: AeoHeroProps) {
      ...
      <div className="relative z-10 mx-auto max-w-[1800px] px-6 pt-20 pb-8 sm:px-10 sm:pt-28 sm:pb-10 lg:px-16 flex flex-col items-start justify-center">
        ...
        <div className="flex flex-col items-start gap-6 text-left max-w-2xl">
          ...
          <h1 className="text-foreground text-[28px] sm:text-[40px] md:text-[48px] leading-[1.15] font-light tracking-tight">
            <span className="block">Get cited by every</span>
            <span className="block">AI answer engine</span>
          </h1>

          {/* Subtext */}
          <p className="text-muted-foreground text-base sm:text-lg font-normal leading-relaxed max-w-[280px] sm:max-w-md">
            ChatGPT, Perplexity, Gemini and Claude recommend you, not them.
          </p>
          ...
          <div className="flex flex-wrap gap-3 items-center mt-2">
            ...
            <button 
              type="button" 
              className="btn-gradient-highlight group relative cursor-pointer inline-flex items-center justify-center gap-0 rounded-full py-1.5 px-5 text-center text-sm font-normal transition-all hover:opacity-90 active:brightness-95 bg-foreground text-background border border-transparent w-auto font-sans"
            >
